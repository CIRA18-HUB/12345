...
<style>
    ...
    :root {
        ...
        --feishu-green: #36B37E;
        --feishu-green-light: #E6F7F1;
        --feishu-red: #F5222D;
        --feishu-red-light: #FFF2F0;
        --feishu-yellow: #FAAD14;
        --feishu-yellow-light: #FFFBE6;
        --feishu-purple: #722ED1;
        --feishu-purple-light: #F9F0FF;
    }
    ...
    /* 商业洞察区域 */
    .business-insight {
        background-color: var(--feishu-green-light);
        padding: 0.9rem;
        border-radius: 6px;
        margin: 0.8rem 0;
        border-left: 3px solid var(--feishu-green);
        font-size: 0.92rem;
        color: var(--feishu-text-primary);
    }
    ...
    /* 行动建议区域 */
    .action-tip {
        background-color: var(--feishu-yellow-light);
        padding: 0.9rem;
        border-radius: 6px;
        margin: 0.8rem 0 1.4rem 0;
        border-left: 3px solid var(--feishu-yellow);
        font-size: 0.92rem;
        color: var(--feishu-text-primary);
    }
    ...
    div[data-testid="stTable"] th {
        background-color: var(--feishu-gray);
        color: var(--feishu-text-primary);
        font-weight: 500;
...
def configure_chart(fig, title, xaxis_title, yaxis_title, height=450, legend_title=None):
    ...
    fig.update_layout(
        title=dict(
            text=title,
            font=dict(size=16, family="Arial, sans-serif", color="#1F2329")
        ),
        xaxis=dict(
        ...
        font=dict(family="Arial, sans-serif", color="#1F2329"),
        hovermode="closest"
    )
    return fig
...
with tabs[0]:  # 销售概览
    ...
    # 添加销售额柱状图
    colors_sales = px.colors.sequential.Blues
    for i, row in applicant_performance.iterrows():
        color_idx = min(len(colors_sales) - 1, i + 3)  # 使用更深的蓝色
    ...
    fig_applicant_performance.update_layout(
        ...
        margin=dict(t=80, b=120, l=60, r=60),  # 增加底部边距以容纳图例
        plot_bgcolor='rgba(0,0,0,0)',
        paper_bgcolor='rgba(0,0,0,0)',
        barmode='group'
    )
